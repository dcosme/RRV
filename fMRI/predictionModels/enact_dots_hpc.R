...
if (file.exists("enact_dots_rest_assoc_sca.RDS")) {
  dots_rest_assoc_sca = readRDS("enact_dots_rest_assoc_sca.RDS")
} else {
  data = dots_sca_enact %>%
    select_if(grepl("subjectID|enact|association", names(.))) %>%
  ...
  saveRDS(dots_rest_assoc_sca, "enact_dots_rest_assoc_sca.RDS")
}

# dots uniformity > rest
if (file.exists("enact_dots_rest_uniform_sca.RDS")) {
  dots_rest_uniform_sca = readRDS("enact_dots_rest_uniform_sca.RDS")
} else {
  data = dots_sca_enact %>%
    select_if(grepl("subjectID|enact|uniformity", names(.))) %>%
    select_if(grepl("subjectID|enact|rest", names(.)))
  lm_predictors = paste(names(select(data, -c(subjectID, enact_prop))), collapse = " + ")
  lm_formula = formula(paste0("enact_prop ~ ", lm_predictors, collapse = " + "))
  ...
  saveRDS(dots_rest_uniform_sca, "enact_dots_rest_uniform_sca.RDS")
}

# dots association > nature
if (file.exists("enact_dots_nature_assoc_sca.RDS")) {
  dots_nature_assoc_sca = readRDS("enact_dots_nature_assoc_sca.RDS")
} else {
  data = dots_sca_enact %>%
    select_if(grepl("subjectID|enact|association", names(.))) %>%
  ...
  saveRDS(dots_nature_assoc_sca, "enact_dots_nature_assoc_sca.RDS")
}

# dots uniformity > nature
if (file.exists("enact_dots_nature_uniform_sca.RDS")) {
  dots_nature_uniform_sca = readRDS("enact_dots_nature_uniform_sca.RDS")
} else {
  data = dots_sca_enact %>%
    select_if(grepl("subjectID|enact|uniformity", names(.))) %>%
    select_if(grepl("subjectID|enact|nature", names(.)))
  ...
  saveRDS(dots_nature_uniform_sca, "enact_dots_nature_uniform_sca.RDS")
}
